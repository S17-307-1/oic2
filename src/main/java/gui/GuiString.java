...
public class GuiString extends GuiElement {
	public static final TrueTypeFont DEFAULT_FONT = new TrueTypeFont(new Font("Dialog", Font.PLAIN, 14), true);
	public static final TrueTypeFont HEADER_FONT = new TrueTypeFont(new Font("Dialog", Font.BOLD, 32), true);
	public static final TrueTypeFont BUTTON_FONT = new TrueTypeFont(new Font("Dialog", Font.BOLD, 28), true);
	...
	public void render(Graphics graphics) {
		...
		for (String str : content) {
			if (isAligned && line > 0) {
				...
				graphics.drawString(str, (float) location.getX(), (float) location.getY() + line * lineHeight);
			}
			line += 1;
		}
	...
	public Rectangle2D getBoundingBox() {
		...
		return new Rectangle2D.Double(location.getX(), location.getY(), maxWidth, font.getHeight() * content.length);
	}
