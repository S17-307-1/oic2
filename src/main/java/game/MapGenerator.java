...
public class MapGenerator {
	...
	private void generateBackgroundTiles() {
		for (int i = 0; i < world.getNumTiles(); i++) {
			...
			if (world.getPlayer().getCenter().distance(currBackgroundTile.getCenter()) <= GENERATOR_RADIUS) {
	...
	private void attemptToGenerateTile(BackgroundTile currBackgroundTile, TileDirection dir) {
		...
		switch (dir) {
		case LEFT:
			newTilePos = new Vector2f(currBackgroundTile.getPosition().getX() - newBackgroundTile.getWidth(),
					currBackgroundTile.getPosition().getY());
			break;
		case RIGHT:
			newTilePos = new Vector2f(currBackgroundTile.getPosition().getX() + currBackgroundTile.getWidth(),
					currBackgroundTile.getPosition().getY());
			break;
		case UP:
