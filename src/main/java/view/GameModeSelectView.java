...
public class GameModeSelectView extends View {
	...
	void initContinue() {
		...
		tempString.shift((int) ((OperationIceCream.WINDOW_SIZE_X - tempString.getBoundingBox().getWidth()) / 2), 10);
		...
		tempButton = new Button("btn-2", "FREE PLAY", 0, (int) (options.getBoundingBox().getHeight() + 10), 250);
		tempButton.setClickEvent(() -> {
			stateBasedGame.enterState(OperationIceCream.FREEPLAY_CONFIG);
		});
		options.addComponent(tempButton);

		tempButton = new Button("btn-3", "MULTIPLAYER", 0, (int) (options.getBoundingBox().getHeight() + 10), 250);
		tempButton.setClickEvent(() -> {
			stateBasedGame.enterState(OperationIceCream.MULTIPLAYER_CONFIG);
		});
		options.addComponent(tempButton);

		tempButton = new Button("btn-4", "TUTORIAL", 0, (int) (options.getBoundingBox().getHeight() + 10), 250);
		tempButton.setClickEvent(() -> {
		...
		options.shift((int) ((OperationIceCream.WINDOW_SIZE_X - options.getBoundingBox().getWidth()) / 2.0), 500);
